    ...
    )
    {
      ...
      if(opts_init.sd_const_multi > 0 && opts_init.src_switch)
        ...
        if (n_dims > 0)
        {
          ...
          if (!(opts_init.z0 >= 0 && opts_init.z0 < std::accumulate(opts_init.vert_stretch_prof.begin(), opts_init.vert_stretch_prof.end(), 0) * opts_init.dz))
...
//            throw std::runtime_error("!(x1 > x0 & x1 <= min(1,nx)*dx)");
          ...
          if (!(opts_init.z1 > opts_init.z0 && opts_init.z1 <= std::accumulate(opts_init.vert_stretch_prof.begin(), opts_init.vert_stretch_prof.end(), 0) * opts_init.dz))
