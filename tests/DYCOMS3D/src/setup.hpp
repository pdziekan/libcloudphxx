...
namespace setup 
{
  ...
  const quantity<si::dimensionless, real_t> kappa_gccn = 1.28; // CCN-derived value from Table 1 in Petters and Kreidenweis 2007
  // for blk_2m:
...
*/
  ...
  void intcond(concurr_t &solver, blitz::Array<setup::real_t, 3> &rhod)
  {
  ...
  void env_prof(blitz::Array<setup::real_t, 3> &th_e, blitz::Array<setup::real_t, 3> &rv_e, blitz::Array<setup::real_t, 3> &th_ref, blitz::Array<setup::real_t, 3> &rhod, int nz)
  {
    ...
    th_e(all, all, 0) = th_l(0.) / si::kelvins;
    rv_e(all, all, 0) = rt(0.);
    ...
    for(int k=1; k<nz; ++k)
    {
      setup::real_t bottom = R_d<setup::real_t>() / si::joules * si::kelvins * si::kilograms * T(k-1) * (1 + 0.61 * rv_e(0, 0, k-1));
      ...
      if(delta < 0.) delta = 0.;
      rv_e(all, all, k) = rt(k*dz) - delta;
      th_e(all, all, k) = th_l(k*dz) / si::kelvins + c * thetme * delta;
      T(k) = th_e(0, 0, k) * pow(pre(k) / (p_1000<setup::real_t>() / si::pascals),  f);
    }
    ...
    st=0;
    st(notopbot) = (th_e(0, 0, notopbot+1) - th_e(0, 0, notopbot-1)) / th_e(0, 0, notopbot);
    setup::real_t st_avg = blitz::sum(st) / (nz-2) / (2.*dz);
    // reference theta
    th_ref = th_e(0,0,0) * exp(st_avg * k * dz);
    ...
    setup::real_t T_surf = th_e(0, 0, 0) *  pow(setup::p_0 / p_1000<setup::real_t>(),  R_d_over_c_pd<setup::real_t>());
    setup::real_t T_virt_surf = T_surf * (1. + 0.608 * rv_e(0, 0, 0));
